...
Google Gemini soporta el suffix `:online` en cualquier modelo.
...
const result = streamText({
  model: google(`${MODELS.balanced}:online`),  // Siempre online
  system: SYSTEM_PROMPT,
  messages: modelMessages,
})
```
...
Todos los modelos de Google Gemini soportan `:online`:
