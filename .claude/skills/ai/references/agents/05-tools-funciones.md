# Bloque 05: Tools y Funciones (AI SDK 6)

> Agregar herramientas que el modelo puede ejecutar.

**Tiempo:** 20 minutos
**Prerequisitos:** Bloque 01 (Chat Streaming)
**Version:** AI SDK 6 (Diciembre 2025)

---

## Que Obtienes

- Tools definidas con Zod schemas (sintaxis AI SDK 6)
...
## 1. Definir Tools con Zod (Sintaxis AI SDK 6)
...
export const getWeather = tool({
  ...
  // AI SDK 6: Usa inputSchema en lugar de parameters
...
## 2. API Route con Tools (AI SDK 6)
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    tools,
    // AI SDK 6: stopWhen reemplaza maxSteps
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    // AI SDK 6: Control dinamico por paso
    prepareStep: async ({ stepNumber, previousSteps }) => {
      // Ejemplo: Cambiar modelo segun complejidad
      if (stepNumber > 2) {
        return {
          model: google(MODELS.powerful),  // Usar modelo mas potente
        }
      }
